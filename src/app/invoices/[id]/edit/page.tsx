...
export default function EditInvoicePage() {
  ...
  // Fetch initial data (available items & the invoice to edit)
  useEffect(() => {
    async function fetchData() {
      if (!user) return;
      
      try {
        // Fetch all available items for the user
        const itemsRes = await fetch(`/api/invoices/items?userID=${user.id}`);
        ...
        // Fetch the invoice
        const invoiceRes = await fetch(`/api/invoices/${invoiceID}?userID=${user.id}`);
        if (!invoiceRes.ok) throw new Error('Failed to fetch invoice');
        const invoiceData = await invoiceRes.json();

        // Populate form
        setBillTo(invoiceData.billTo || '');
        setBillToAddressLine(invoiceData.billToAddressLine || '');
        setBillToCityAddress(invoiceData.billToCityAddress || '');
        setBillToPostalAddress(invoiceData.billToPostalAddress || '');
        setBillToCountry(invoiceData.billToCountry || '');
        setDueDate(invoiceData.dueDate || '');
        setCurrency(invoiceData.currency as 'PHP' | 'USD');
        setTaxRate((invoiceData.taxRate * 100).toString());
        setTerms(invoiceData.terms || '');

        // Populate selected item IDs
        const preSelected = new Set<string>();
        if (invoiceData.invoiceItems) {
          invoiceData.invoiceItems.forEach((item: InvoiceItem) => {
            preSelected.add(item.itemID);
          });
        }
        ...
        if (invoiceData.invoiceItems) {
          invoiceData.invoiceItems.forEach((item: InvoiceItem) => {
            if (!existingItemMap.has(item.itemID)) {
              existingItemMap.set(item.itemID, item);
            }
          });
        }
        setAvailableItems(Array.from(existingItemMap.values()));
