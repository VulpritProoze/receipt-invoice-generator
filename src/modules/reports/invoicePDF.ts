...
export async function generateInvoicePDF(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      doc.fontSize(20).font('Helvetica-Bold').text(companyConfig.brandName, 50, 50);
      ...
      doc.fillColor('blue').text(companyConfig.companyUrl, 50, yPos, { underline: true });
      doc.fillColor('black');
      yPos += 15;
      ...
      doc.font('Helvetica-Bold').text('Invoice #', rightX, rightYPos, { width: 80 });
      doc.font('Helvetica').text(invoice.invoiceID, rightX + 85, rightYPos, { width: 77 });
      rightYPos += 15;

      doc.font('Helvetica-Bold').text('Invoice Date', rightX, rightYPos, { width: 80 });
      doc
        .font('Helvetica')
        .text(new Date(invoice.invoiceDate).toLocaleDateString(), rightX + 85, rightYPos, {
          width: 77,
        });
      rightYPos += 15;

      doc.font('Helvetica-Bold').text('Terms', rightX, rightYPos, { width: 80 });
      doc.font('Helvetica').text(invoice.terms, rightX + 85, rightYPos, { width: 77 });
      rightYPos += 15;

      doc.font('Helvetica-Bold').text('Due Date', rightX, rightYPos, { width: 80 });
      doc
        .font('Helvetica')
        .text(new Date(invoice.dueDate).toLocaleDateString(), rightX + 85, rightYPos, {
          width: 77,
        });
      rightYPos += 15;

      doc.font('Helvetica-Bold').text('Currency', rightX, rightYPos, { width: 80 });
      doc.font('Helvetica').text(invoice.currency, rightX + 85, rightYPos, { width: 77 });
      ...
      doc.text('QUANTITY', colQuantity, tableTop, { width: 60, align: 'right' });
      ...
      for (const item of invoice.invoiceItems) {
        ...
        doc.text(item.quantity.toString(), colQuantity, yPos, { width: 60, align: 'right' });
        doc.text(item.description, colDescription, yPos, { width: 250 });
        doc.text(formatAmount(item.rate), colRate, yPos, { width: 90, align: 'right' });
      ...
      // Subtotal
      doc.font('Helvetica-Bold').text('SUBTOTAL:', colRate, yPos, { width: 90, align: 'right' });
      doc
        .font('Helvetica')
        .text(formatAmount(subtotal), colAmount, yPos, { width: 82, align: 'right' });
      yPos += 18;

      // Tax
      const taxAmount = subtotal * invoice.taxRate;
      const taxLabel = `TAX (${(invoice.taxRate * 100).toFixed(0)}%):`;
      doc.font('Helvetica-Bold').text(taxLabel, colRate, yPos, { width: 90, align: 'right' });
      doc
        .font('Helvetica')
        .text(formatAmount(taxAmount), colAmount, yPos, { width: 82, align: 'right' });
      yPos += 18;
      ...
      doc.text(formatAmount(invoiceTotal), colAmount, yPos, { width: 82, align: 'right' });
