...
5. Contract tests
6. Fixture tests
7. Security / guardrails tests
8. Linting check — final       (ESLint, must still be clean after test setup)
```
...
**If Step 2 (initial lint) fails:** stop. Do not run Steps 3–8. Fix lint errors using the `lint` skill. Then restart from Step 1.

**If Steps 3–7 have failures:** record every failure and continue running the remaining steps unless a failure is a hard blocker (e.g., a missing module import that prevents the entire test runner from loading). The goal is to collect all failures in one pass so they can all be fixed in one `/test-fix-iterate` cycle.

**If Step 8 (final lint) fails:** test setup code introduced a lint regression. Fix it. A suite that passes Steps 3–7 but fails the final lint check is still a red suite.
...
**Redis (Upstash):** Never call real Upstash in any test. Use a Jest manual mock in `src/lib/__mocks__/redis.ts` that implements an in-memory key-value store. The mock resets between test cases in `beforeEach`. Contract tests, fixture tests, and security tests all use this mock.
