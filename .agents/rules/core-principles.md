...
Every non-obvious decision in this codebase is traceable to either an inline comment (explaining the *why*, not the *what*) or an ADR in `docs/decisions/` linked from the relevant code.
...
- Something was deliberately *not* implemented that might be expected.
